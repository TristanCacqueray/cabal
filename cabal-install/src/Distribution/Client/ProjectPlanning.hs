      ...
      where
        ...
        buildComponent (cc_map, lc_map, exe_map) comp =
          addProgressCtx (text "In the stanza" <+>
            ...
            -- MP: Think False here is correct? It dictates whether we have any promised
            -- packages on the go or not.
...
pruneInstallPlanPass1 pkgs
  ...
  where
    ...
    all_desired_repl_targets = [cp | InstallPlan.Configured cp <- fromMaybe [] $ Graph.revClosure closed_graph roots]

    add_repl_target :: ElaboratedConfiguredPackage -> ElaboratedConfiguredPackage
    add_repl_target ecp | ecp `elem` all_desired_repl_targets = ecp { elabReplTarget = maybeToList (ComponentTarget <$> (elabComponentName ecp) <*> pure WholeComponent)
                                                                    , elabBuildStyle = BuildInplaceOnly InMemory }
                        | otherwise = ecp
