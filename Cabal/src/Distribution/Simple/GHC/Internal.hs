...
-- | Create the require packaged arguments, but filtering out package arguments which
...
mkGhcOptPackages promised_pkgs clbi = [ i | i@(uid, _) <- componentIncludes clbi
                                          , abstractUnitId uid `Set.notMember` promised_cids ]
  where
    -- Promised deps are going to be simple UnitIds
    promised_cids = Set.fromList (map newSimpleUnitId (Map.elems promised_pkgs))
